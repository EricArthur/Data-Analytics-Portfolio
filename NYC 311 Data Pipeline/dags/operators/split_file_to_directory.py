...
class SplitFileToDirectoryOperator(BaseOperator):
    ...
    def execute(self, context):
        ...
        # Split file into multiple ones under directory
        self.log.info('Writing files')
        for result in json_input:
            rendered_json_output_filepath = rendered_output_directory + self.json_output_filepath.format(unique_id=result['unique_id'])
